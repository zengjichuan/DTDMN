...
class GaussianKLLoss(_Loss):
    ...
    def forward(self, mu, logvar, mask=None):
        """
        0.5 * sum(1 + log(sigma^2) - mu^2 - sigma^2)
        """
        y_kl = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp(), dim=2)

        if mask is None:
            return torch.mean(y_kl)
        else:
            y_kl *= mask
            return torch.sum(y_kl) / torch.sum(mask)


class MultivariateGaussianKLLoss(_Loss):
    def __init__(self):
        super(MultivariateGaussianKLLoss, self).__init__()

    def forward(self, mu_q, log_cov_q, mu_prior, log_cov_prior, mask=None):
        """
        0.5*(log|sigmasq_p| -log|sigmasq_q|  -D + Tr(sigmasq_p^-1 sigmasq_q)
                        + (mu_p-mu_q)^T sigmasq_p^-1 (mu_p-mu_q))
        :param input:
        :return:
        """
        diff_mu = mu_prior - mu_q
        y_kl = 0.5 * torch.sum(log_cov_prior - log_cov_q - 1. + (log_cov_q - log_cov_prior).exp() +
                               diff_mu.pow(2) / log_cov_prior.exp(), dim=2)
        if mask is None:
            return torch.mean(y_kl)
        else:
            y_kl *= mask
...
class BCELoss(_Loss):
    ...
    def forward(self, pred, target, mask=None):
        # pred_sigm = self.sigm(pred)
        # print("max: %f, min: %f" % (torch.max(pred_sigm).item(), torch.min(pred_sigm).item()))
        if mask is None:
            return torch.mean(self.loss_fn(pred, target))
        else:
...
class PPLLoss(_Loss):
    ...
    def forward(self, log_qy, target, mask=None):
        """
        - py * log(q(y))
        """
        if self.weight is not None and self.ignore is None:
            y_ppl = - torch.sum(self.weight * target * log_qy, dim=2)
        elif self.ignore is not None and self.weight is None:
            y_ppl = - torch.sum(self.ignore * target * log_qy, dim=2)
        elif self.weight is not None and self.ignore is not None:
            y_ppl = - torch.sum(self.weight * self.ignore * target * log_qy, dim=2)
        else:
            y_ppl = - torch.sum(target * log_qy, dim=2)
        if mask is None:
            return torch.mean(y_ppl)
        else:
            y_ppl *= mask
            return torch.sum(y_ppl) / torch.sum(mask)
