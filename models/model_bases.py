...
class BaseModel(nn.Module):
    ...
    def backward(self, batch_cnt, loss, annealing_factor):
        total_loss = self.valid_loss(loss, batch_cnt, annealing_factor)
        total_loss.backward()
    ...
    def get_optimizer(self, config):
        if config.op == 'adam':
            ...
            return torch.optim.Adam(filter(lambda p: p.requires_grad,
                                           self.parameters()), lr=config.init_lr, weight_decay=config.weight_decay)
        elif config.op == 'sgd':
            print("Use SGD")
